...
public class TransactionService {
    ...
    public Transaction processNewTransactionRequest(TransactionRequestDTO request) {
        ...
        ClientAccountCheckResponseDTO clientCheckResponse;
        try {
            clientCheckResponse = checkClientWithService2(clientId, account.getId());
        } catch (Exception e) {
        ...
        return webClient.post()
                ...
                .onStatus(status -> status.isError(), response ->
                        response.bodyToMono(String.class)
                                .defaultIfEmpty("Ошибка при проверке клиента")
                                .flatMap(body -> Mono.<Throwable>error(new RuntimeException("Service 2 failed: " + body)))
                )
